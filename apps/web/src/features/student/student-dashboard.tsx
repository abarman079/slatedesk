...
export function StudentDashboardView() {
  ...
  const assignments = useQuery({
    ...
    queryFn: () =>
      request<PagedResult<StudentAssignment>>(
  ...
  const results = useQuery({
    ...
    queryFn: () =>
      request<PagedResult<StudentResult>>(
        "/api/v1/student/results?page=1&pageSize=100",
      ),
  });

  const error = assignments.error ?? submissions.error ?? results.error;

  if (assignments.isLoading || submissions.isLoading || results.isLoading) {
  ...
  const dueSoon = assignmentItems
    .filter(
      (item) => !item.isPastDeadline && item.submissionStatus !== "Graded",
    )
    .sort(
      (a, b) =>
  ...
  const pastDeadline = assignmentItems.filter(
    (item) => item.isPastDeadline && item.submissionStatus !== "Graded",
  ).length;
  ...
  const progress =
    assignmentItems.length === 0
      ? 0
      : Math.round((submittedIds.size / assignmentItems.length) * 100);
    ...
    <>
      ...
      <section className="student-stat-grid">
        <Card className="student-stat">
          <span className="overview-label">Due soon</span>

          <strong>{dueSoon.length}</strong>
        </Card>

        <Card className="student-stat">
          <span className="overview-label">Past deadline</span>

          <strong>{pastDeadline}</strong>
        </Card>

        <Card className="student-stat">
          <span className="overview-label">Submitted</span>

          <strong>{submittedIds.size}</strong>
        </Card>

        <Card className="student-stat">
          <span className="overview-label">Graded</span>
      ...
      >
        <div className="dashboard-chart-heading">
          <div>
            <p className="eyebrow">Workload map</p>

            <h2>Academic progress</h2>
          </div>

          <span className="dashboard-chart-note">Current assignment state</span>
        </div>

        <DashboardChart
          data={chartData}
          ariaLabel="Student academic progress summary"
        />
      </Card>
      ...
      <section className="student-dashboard-grid">
        <Card className="student-progress-card">
          ...
          <div className="student-progress-value">{progress}%</div>

          <p className="muted">
            of currently visible assignments have a submitted response.
          </p>
          ...
          <p className="student-progress-note">
            Progress reflects submitted academic work, not a final course grade.
          </p>
        </Card>
        ...
        <Card>
          ...
          {latestResult ? (
            <>
              ...
              >
                <GradeSeal
                  marks={latestResult.marksAwarded}
                  maximumMarks={latestResult.maximumMarks}
                />
              </div>
      ...
      >
        ...
        >
          <div>
            <p className="eyebrow">Next in your ledger</p>
